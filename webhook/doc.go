...
//
// Type safety: Webhook plus typed Options plus Adapter enum plus Factory. Options carry Timeout plus MaxRetries plus QueueAdapter plus QueueOpts plus DeadLetterTopic plus DSN; there is no shared APIKey by design. Unsupported features fail closed.
//
...
//
// Security: never log secrets or raw option maps, use config.RedactedServices. Endpoint secrets are per target at Register time and AllowPrivateTargets stays false in production.
//
