...
Go multi-module monorepo (`go 1.27.0`). Schema-driven scaffolding compiler (`.zen` DSL) + swappable backend adapters. Layers: `core/<b>` = battery interface + registry (`core/<b>.Register/Open`, container resolves); `adapters/<b>/<a>` = one Go module per adapter with `Register()` wiring itself into its `core` registry; `shared/*` = SDK-free helpers (codec, redisclient, s3opts, retry); plus `config|container|orm|dsl|cmd/zever` modules. Dev workflow via `go.work` (143 uses, committed at root). Lockstep releases: one version, per-module tags `<path>/vX.Y.Z` via `tools/tag-release.sh`. Scaffold floor = `log, router` (`generate_server.go` coreBatteries); `zever new` battery picker adds more; `zever add` is wired (`cmd/zever/add.go`).
...
go test -race ./config/...
go test ./dsl/compile/ -run TestName -update  # regen committed backend output only
```
...
- `core/<b>` batteries (`core/cache/`, `core/queue/`, `core/session/`, `core/payment/`, `core/storage/`, …) — one small interface + `Register`/`Open` registry each. Container is sole resolver. Don't add methods to public interfaces (breaks implementers); prefer new interface.
- `dsl/` — `.zen` compiler frontend, internal-only (runtime must not import): `lexer → parser → ast → resolver → ir.Schema`, `compile → backend.Generate`. Parser never panics (best-effort + diagnostics + depth guard). Canonical fixture `dsl/compile/testdata/app.zen`.
...
- Releases bump every version pin together (root, LSP, editors, docs — file list: `.github/CONTRIBUTING.md` release checklist); tag both `vX.Y.Z` and `tools/zever-lsp/vX.Y.Z`. `dsl` has no release version (schema `v1`/`v2` are API versions).
