...
func TestLoggerReceivesReloadFailure(t *testing.T) {
	t.Parallel()
...
func TestNewWiresOptionsLogger(t *testing.T) {
	t.Parallel()

	caplog := &captureLogger{}
	f, err := New(flag.Options{Logger: caplog})
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	...
	if d.log() != caplog {
		t.Fatal("New did not wire Options.Logger into driver")
	}
}
