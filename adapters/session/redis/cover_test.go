...
// expiry/corruption scenarios the public API cannot construct. addr must
// match the store's own Redis.Addr to observe the same backend.
func plantRaw(t *testing.T, addr, prefix, id, raw string) {
	t.Helper()

	if err := rawClient(t, addr).Set(t.Context(), prefix+":"+id, raw, time.Hour).Err(); err != nil {
...
func TestCoverGetAppExpiredDeletes(t *testing.T) {
	...
	plantRaw(t, opts.Redis.Addr, opts.Redis.Prefix, id, wireJSON(t, time.Now().Add(-time.Hour)))

	if _, err := st.Get(ctx, id); !errors.Is(err, session.ErrNotFound) {
		t.Fatalf("Get(app-expired) = %v, want ErrNotFound", err)
	}
	if n, _ := rawClient(t, opts.Redis.Addr).Exists(ctx, opts.Redis.Prefix+":"+id).Result(); n != 0 {
...
func TestCoverSaveOverCorruptUpsertsFresh(t *testing.T) {
	...
	plantRaw(t, opts.Redis.Addr, opts.Redis.Prefix, id, "{corrupt")
...
func TestCoverSaveZeroExpiryPersists(t *testing.T) {
	...
	plantRaw(t, opts.Redis.Addr, opts.Redis.Prefix, id, wireJSON(t, time.Time{}))
	...
	ttl, err := rawClient(t, opts.Redis.Addr).TTL(ctx, opts.Redis.Prefix+":"+id).Result()
	if err != nil {
		t.Fatalf("TTL: %v", err)
	}
	if ttl != -1 {
