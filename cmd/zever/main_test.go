...
func TestSubcommandHandlers_coversEverySubcommand(t *testing.T) {
	want := []string{
		"new", "compile", "doctor", "routes", "check", "breaking", "fmt",
		"explain", "check-boundaries", "graph", "generate", "extract",
...
func TestRun_helpVariants_returnNil(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"help routes (no dedicated usage)", []string{"help", "routes"}},
		{"help explain (no dedicated usage)", []string{"help", "explain"}},
		{"help check-boundaries (no dedicated usage)", []string{"help", "check-boundaries"}},
		{"help graph (no dedicated usage)", []string{"help", "graph"}},
